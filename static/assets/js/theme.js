...
var chatInit = function chatInit() {
  ...
  // BOX MESSAGE
  if ($chatEmojiarea) {
    $chatEmojiarea.setAttribute('placeholder', 'Aa');
    $chatEmojiarea.focus()
    ...
    if (tinymces.length) {
      window.tinymce.init(
      {
        ...
        setup: function(editor) {
          ...
          var textareaElement = document.querySelector(`#${tinymceInstanceId}`);
          tinymceInstances.forEach(function(instanceElement) {
            var tinymceInstanceId = instanceElement.id;
            var tinymceEditor = tinymce.get(tinymceInstanceId);
            if (tinymceEditor) {
              var content = tinymceEditor.getContent();
              var tempElement = document.createElement('div');
              tempElement.innerHTML = content;
              var paragraphs = tempElement.querySelectorAll('p');
              paragraphs.forEach(function(paragraph) {
                var paragraphContent = paragraph.innerHTML;
                textareaElement.value = paragraphContent;
              });
            }
          })
        })}
